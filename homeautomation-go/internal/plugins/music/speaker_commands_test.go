...
func setupSoCoForTest(t *testing.T, manager *Manager, readOnly bool) *threadSafePaths {
	...
	// Use zap.NewNop() for the SoCo client logger to avoid data races.
	// Background goroutines (fade-in, retry) may log after the test ends;
	// zaptest.NewLogger(t) would race on t's state in that case.
