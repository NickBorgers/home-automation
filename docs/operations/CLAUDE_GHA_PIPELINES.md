...
|----------|------|---------|---------|
| Codex | `claude.yml` | @codex mentions, new issues | Respond to requests, auto-resolve issues |
| Codex Code Review | `claude-code-review.yml` | PR Tests completion, PR reopened | Multi-agent code review, fix failures, merge decision |
| PR Tests | `pr-tests.yml` | PRs, pushes to claude/** | Run tests, trigger review pipeline |
| Codex Diagnose Workflow Failure | `claude-diagnose-workflow-failure.yml` | Any workflow failure | Diagnose Actions config problems (not test failures) |
...
## 2. Codex Code Review (`claude-code-review.yml`)
...
## 4. Codex Diagnose Workflow Failure (`claude-diagnose-workflow-failure.yml`)
...
Checks if the devcontainer image already exists in GHCR and only builds if missing. Since this is a monthly job, it relies on other workflows (`claude.yml`, `pr-tests.yml`) to keep the image up to date.
...
**Issue Integration**: Issues are created with `@codex` in the title prefix, so the `resolve-issue` job in `claude.yml` automatically picks them up and creates fix PRs.
...
|--------|---------|---------|
| `OPENAI_API_KEY` | claude.yml, claude-code-review.yml, claude-diagnose-workflow-failure.yml, ha-deprecation-check.yml | Codex authentication via LiteLLM |
...
|-------------|-------|-----------|
| `claude.yml` interactive and issue resolution | `gpt-5.4` | Open-ended implementation and repo changes |
| `claude-code-review.yml` review and merge jobs | `gpt-5.4` | Consistent review quality across the pipeline |
| `claude-diagnose-workflow-failure.yml` | `gpt-5.4` | Failure triage and classification |
